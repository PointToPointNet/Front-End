import style from "../styles/all-total-connect.module.scss"
...
const AllTotalConnect: React.FC<WebData> = ({ webData }) => {
  ...
  const drawGraph = (svgRef: React.RefObject<SVGSVGElement>, data: WebData) => {
  ...
  useEffect(() => {
    drawGraph(memoryRef, webData); // 새로운 데이터 전달
  }, [webData]);

  return (
    <div className={style.body} id="memorybox">
      <h2 className={style.title}>💻 Daily Connect Count Data (%) </h2>
