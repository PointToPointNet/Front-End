...
import style from "../styles/all-total-packet.module.scss";
...
const AllTotalpacket: React.FC = ({packetData}) => {
  const packetRef = useRef<SVGSVGElement | null>(null);
  const colorValues = ["#0c9b5d", "#135E93", "#e33535", "#1a9aae", "#da7421"]; // 서버별 색상
  const [helperVisible, setHelperVisible] = useState(false);
  const drawGraph = (svgRef: React.RefObject<SVGSVGElement>, data: PacketData) => {
    if (!svgRef.current || Object.keys(data).length === 0) return;

    const width = parseInt(d3.select("#allpacket").style("width"), 10) - 20;
    const height = 300;
  ...
  useEffect(() => {
    drawGraph(packetRef, packetData); // 새로운 데이터 전달
  }, [packetData]);
    ...
    <div className={style.body} id="allpacket">
      ...
      >
        <p className={style.help}>
          각 서버별 패킷(Packet) 송수신량을 한눈에 보여줍니다.
        </p>
